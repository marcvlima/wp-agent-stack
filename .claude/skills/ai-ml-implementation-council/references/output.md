...
## 5. AI/ML-specific mandatory artifacts (when the brief needs them)

When the decision involves serving, training, or hardware (almost always for this council),
the deliverable **must** also include:

### 5a. Topology block
Text topology for the recommended option, e.g.:

```
Host: Linux + 1× RTX 3090 24GB (PCIe 4.0 x16)
Layout: single-GPU, no TP
Interconnect: n/a
Engine: <name> <version>
Model: <id> <quant>
```
...
```
2× GPU, TP=2, NVLink=yes|no, P2P=measured|blocked
```

### 5b. Executable configuration
Exact flags/env/YAML — not “tune as needed”. Include:

- Engine CLI argv or config file fragments
- Parallel sizes (TP/PP/EP/DP) if any
- Memory knobs (gpu_memory_utilization, max_model_len, kv dtype, MLX limits, …)
- Draft/MTP settings if used
- OS/driver/CUDA or Metal constraints that are load-bearing

### 5c. Acceptance probes
Commands that prove the config works on the **named** host class:

- Decode tok/s probe (min tokens, thinking off/on as relevant)
- Prefill / TTFT probe at representative prompt sizes
- OOM / thrash smoke (max context claim)
- Tool-call or coding smoke if that is the workload
- Explicit **pass/fail thresholds**

### 5d. Workload matrix (when multi-use)
If one stack serves multiple workloads (e.g. coding agents + chat), state what is shared
vs what must be split (second GPU, second process, different quant).
